// ======================= main.cpp =======================
// Vulkan + GLFW + ECS: plane, cube, dummy.glb, WASD + mouse + Xbox
...
void addPlaneAndCubeGeometry(uint32_t& planeMaterialID, uint32_t& cubeMaterialID) {
    gVertices.clear();
    gIndices.clear();
...
// ----------------------------------------------------------
// Upload shared vertex/index buffers
// ----------------------------------------------------------
...
// ----------------------------------------------------------
// Minimal dummy.glb loader (single mesh, single baseColorTexture)
// ----------------------------------------------------------
...
GLTFLoadedMesh loadDummyGLB(const char* path, uint32_t& outMaterialID) {
    ...
    if (prim.material && prim.material->pbr_metallic_roughness.base_color_texture.texture) {
        ...
        if (tex->image && tex->image->uri) {
            std::string texPath = tex->image->uri;
            LoadedImage img = loadPNG(texPath.c_str());
    ...
    gMaterials.push_back({ texView, texSampler });
...
// ----------------------------------------------------------
// Descriptor writes (bind plane/cube texture to binding 1)
// ----------------------------------------------------------

void updateDescriptorSets(uint32_t materialIDForUBO) {
    for (size_t i = 0; i < gGlobalDescSets.size(); ++i) {
        ...
        // For now, bind plane/cube texture as global; dummy.glb uses its own sampler via materialID in shader if needed.
        Material& mat = gMaterials[materialIDForUBO];
        ...
        writes[0].dstSet = gGlobalDescSets[i];
        writes[0].dstBinding = 0;
        writes[0].descriptorType = vk::DescriptorType::eUniformBuffer;
        writes[0].descriptorCount = 1;
        writes[0].pBufferInfo = &bi;

        writes[1].dstSet = gGlobalDescSets[i];
        writes[1].dstBinding = 1;
        writes[1].descriptorType = vk::DescriptorType::eCombinedImageSampler;
        writes[1].descriptorCount = 1;
        writes[1].pImageInfo = &ii;

        gDevice.updateDescriptorSets((uint32_t)writes.size(), writes.data(), 0, nullptr);
    }
}
...
int main() {
    try {
        ...
        gMaterials.push_back({ gPlaneTexView, gPlaneTexSampler });

        // Geometry: plane + cube
        uint32_t planeMatID = 0;
        uint32_t cubeMatID = 0;
        addPlaneAndCubeGeometry(planeMatID, cubeMatID);
        ...
        // Bind plane/cube material as global texture in descriptor sets
        updateDescriptorSets(planeMatID);
        ...
        planeM.vertexOffset = 0;
        planeM.indexOffset = 0;
        planeM.indexCount = 6;
        planeM.materialID = planeMatID;
        MaterialComponent planeMat{};
        planeMat.materialID = planeMatID;
        ...
        MeshComponent cubeM{};
        cubeM.vertexOffset = 4; // after plane's 4 verts
        cubeM.indexOffset = 6; // after plane's 6 indices
        cubeM.indexCount = (uint32_t)gIndices.size() - cubeM.indexOffset;
        cubeM.materialID = cubeMatID;
        MaterialComponent cubeMat{};
        cubeMat.materialID = cubeMatID;
        ...
        dummyT.position = glm::vec3(0.0f, 0.0f, 15.0f);
        dummyT.scale = glm::vec3(1.0f);
